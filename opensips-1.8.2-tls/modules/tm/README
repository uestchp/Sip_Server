...
Bogdan-Andrei Iancu
   ...
   Revision History
   Revision $Revision: 8740 $ $Date: 2009-07-20 17:37:03 +0300
                              (Lu, 20 iul 2009) $
...
1.4.2.  t_relay([flags])
   ...
   decimal or hexa format; supported flags are:
     * 0x01 - do not generate an 100 trying provisional reply when
       ...
       script. Note: If the transaction was previously created 'by
       hand' with t_newtran() , t_relay will not attempt to send a
       100 Trying provisional reply. The 100 Trying reply is
       generated as part of the transaction building, so if in
       your script you manually take care of creating the
       transaction, you must also take care of sending 100
       provisional replies
